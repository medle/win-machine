...
namespace WinMachine.App
{
  public class MachineDevice : IDisposable
  {
    ...
    private const string DeviceProbingQuery = "HELLO" + CRLF;
    private const string DeviceProbingReply = "OK: InductorMachine ready." + CRLF;
    ...
    {
      if (!IsOpen) {
        serial = SerialConnection.Find(DeviceProbingQuery, DeviceProbingReply);
        if (serial == null) throw new Exception("InductorMachine device not found");
        serial.NewLine = CRLF;
      }
    }
    ...
    public string StartPWM(int micros, int duty1024, int deadClocks) 
      => RunCommand($"PWM {micros} {duty1024} {deadClocks}");
